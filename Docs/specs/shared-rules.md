# Shared rules for all Rewound iOS feature-track agents

Repo: /Users/eytanerez/Documents/GitHub/ios (git; committed baseline includes design system, RewoundKit, shell + auth). Backend contract: /Users/eytanerez/Documents/GitHub/Backend/docs/mobile-api.md. Local backend: http://localhost:8000 (live, seeded, real Stripe TEST keys). Test account: iosbuyer.rewound@gmail.com / iosbuyer / RewoundiOS123!.

## Ownership & no-collision rules (multiple agents work this repo concurrently)
- You own ONLY the Feature directories named in your spec. Never edit: Rewound/App/, Features/Auth/, Features/Onboarding/, Packages/* (RewoundKit + RewoundDesign are read-only APIs to you), project.yml, xcconfigs, other tracks' Feature dirs.
...
- If RewoundKit lacks an endpoint/model you need: add a NEW file under Packages/RewoundKit/Sources/RewoundKit/Models/ or extend a store via a NEW Swift file with an `extension` (never modify existing kit files), matching kit conventions, with a decode test if a fixture exists. List every kit addition in your final message.
...
- Build with YOUR OWN derived data dir and YOUR OWN simulator (named in your spec):
  xcodebuild -project Rewound.xcodeproj -scheme Rewound -destination 'platform=iOS Simulator,name=<YOUR SIM>' -derivedDataPath /tmp/dd-<track> -quiet build
...
- Tokens/components only: RewoundType, Color.rewound.*, Space/Radius/Elevation, Motion.ease (160/220/420ms, ease-out only, no springs/bounce), Haptics.shared vocabulary, existing components (ListingCard, SearchField, FilterChip, SheetScaffold, Toast, EmptyState, SkeletonShimmer, CountdownChip, TimelineRow, ProgressCheckpoints, StatusBadge, SpecList, CalloutBand, StarRating, PriceRangeSlider, PhotoSlotRing, RewoundTextField, SegmentedTabs, AvatarInitial…). No raw hex, no system-blue defaults, no .animation(.spring).
...
- Images: NukeUI LazyImage with downsampling sized to the container; square wells on Color.rewound.secondary.opacity(0.5).

## Verification bar (your final message must report all)
1. Build green with your derivedDataPath.
2. RewoundKit tests still pass: xcodebuild test -only-testing:RewoundKitTests (same destination/derived data).
