...
You own: Rewound/Features/Discover/ only. Replace P2's placeholder root in place.
...
- **Stack**: ZStack of top 3 cards; card = large rounded (Radius.overlay) image-forward layout — watch photo fills ~70% (LazyImage, downsampled), bottom panel: Eyebrow brand·year, serif title, price (RewoundType.price), condition pill. Under-cards at scale 0.97/0.94, offset 10/20pt, slightly dimmed.
...
## Verify
Guest: swipe passes persist (relaunch, same cards don't return), save triggers AuthGateSheet, after sign-in (iosbuyer.rewound@gmail.com / RewoundiOS123!) intent replays and the watch lands in the watchlist (confirm via GET /watchlist curl). Tap-through to PDP route works (PDP itself may still be a placeholder — that's fine, route must fire). Undo restores. Screenshots light+dark: deck resting, mid-drag with SAVE affordance visible, empty state. Read the screenshots and fix visual issues before finishing.
