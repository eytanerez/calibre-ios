...
/// fitted to the drawn centerline rather than eyeballed, and re-tracing it here
...
public struct RewoundLogoMark: View {
    ...
    public var body: some View {
        ZStack {
            Self.strokes.stroke(Color.rewound.primary, style: MarkGrid.style)
            Self.jewelCentre.fill(Color.rewound.primary)
        }
        .markCanvas(size)
        .accessibilityHidden(true)
    }

    /// Everything but the jewel's filled center.
    ///
    ...
    ///
    /// The arc's center form below is that endpoint arc converted, not a new
    /// fit: it round-trips onto 94.52 85.34 and 94.67 31.55 exactly, and the
    /// center it derives lands within a grid unit of the jewel's. The semi-axes
    /// are about two percent apart, so the arc leaves a same-center circle by
